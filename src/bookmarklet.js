...
(function () {
    ...
    const appCode = function () {
            ...
            analyzeText: function (text) {
                ...
                if (words.length === 0 || sentences === 0) {
                    return {
                        ...
                        wordsPerSentence: "0",
                        readabilityScore: "0",
                        interpretation: "Texto muy corto",
        ...
        const MarkdownConverter = {
            convert: function (markdown) {
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    if (/^### /.test(line)) {
                        if (inList) { processedLines.push(this.closeList(listItems, listType)); inList = false; listItems = []; }
                    ...
                    if (!processed) {
                        if (!inList) processedLines.push(line);
                    }
                }
        ...
        const safeSetHTML = (element, html) => {
            try {
                ...
                try {
                    ...
                    console.error('HTML set failed', err);
                }
            }
        };
        ...
        const triggerSave = function () {
            ...
            if (fileName === null) {
                showActionFeedback('Guardado cancelado', 'error');
                return;
            }
        ...
        const copyPlainText = function () {
            ...
            navigator.clipboard.writeText(text)
                .then(() => showNotification('Texto plano copiado'))
        ...
        const copyRichText = function () {
            ...
            navigator.clipboard.write(clipboardData)
                .then(() => showNotification('Texto enriquecido copiado'))
                .catch(() => showNotification('Error al copiar', 'error'));
        };
        ...
        const convertMarkdownAndCopy = function () {
            ...
            navigator.clipboard.write(clipboardData)
                .then(() => showNotification('Convertido y Copiado'))
                .catch(() => showNotification('Convertido'));
        };
    ...
    const styles = `
        ...
        [data-theme="dark"] {
            --bg-color:#1a1a1a;
            --text-color:#e0e0e0;
            --header-bg:rgba(30,30,30,.95);
            --header-border:#444;
            --notepad-bg:#2d2d2d;
            --notepad-text:#e0e0e0;
            --panel-bg:rgba(30,30,30,.97);
            --panel-text:#e0e0e0;
            --stat-bg:#383838;
            --stat-text:#aaa;
            --stat-value:#64b5f6;
        ...
        #toggleTheme {
            ...
            border:1px solid var(--stat-text);
            color:var(--stat-text);
            border-radius:5px;
            transition:all .2s ease;
        }
        ...
        #fixedPanel {
            ...
            z-index:1000;
            padding:.3rem;
            transition:transform .3s ease,max-height .3s ease,background-color .3s;
            max-height:30vh;
            overflow-y:auto;
        }
        ...
        #copyPlainButton { background-color:#4CAF50; }
        #copyRichButton { background-color:#2196F3; }
        #undoButton { background-color:#607D8B; }
        #clearButton { background-color:#F44336; }
        #convertMarkdownButton { background-color:#9C27B0; }
        #saveMarkdownButton { background-color:#ff9800; }
        .notification {
            position:fixed;
            top:60px;
            left:50%;
